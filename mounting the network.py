...
WoK = epub.read_epub('books/WoK.epub')
WoR = epub.read_epub('books/WoR.epub')
OB = epub.read_epub('books/OB.epub')
RoW = epub.read_epub('books/RoW.epub')

list_of_books = [WoK, WoR, OB, RoW]
list_of_books_str = ["WoK", "WoR", "OB", "RoW"]

books = [list(book.get_items_of_type(ebooklib.ITEM_DOCUMENT)) for book in list_of_books]

#books_chapters = [chapter for item in books_chapters for chapter in item]
...
for book_number, book in enumerate(books):
    ...
    G = nx.MultiGraph()
    book_name = list_of_books_str[book_number]
    ...
    # To save the graph
    nx.write_gpickle(G, f"graphs/{book_name}.gpickle")
    ...
    filename = f"tables/{book_name}_nodes_table.csv"
    ...
    filename = f"tables/{book_name}_edges_table.csv"
    df2.to_csv(filename)
